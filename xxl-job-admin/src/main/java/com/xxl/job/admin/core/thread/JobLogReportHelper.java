            ...
            while (!toStop) {
                ...
                try {
                    for (int i = 0; i < 3; i++) {
                        ...
                        // do refresh
                        int ret = XxlJobAdminConfig.getAdminConfig().getXxlJobLogReportDao().update(xxlJobLogReport, new UpdateWrapper<XxlJobLogReport>().lambda().eq(XxlJobLogReport::getTriggerDay, xxlJobLogReport.getTriggerDay()));
                        if (ret < 1) XxlJobAdminConfig.getAdminConfig().getXxlJobLogReportDao().insert(xxlJobLogReport);
                    }
                } catch (Exception e) {
                    if (!toStop) logger.error(">>>>>>>>>>> xxl-job, job log report thread error", e);
                }
                ...
                if ( XxlJobAdminConfig.getAdminConfig().getLogretentiondays() > 0 && System.currentTimeMillis() - lastCleanLogTime > 24*60*60*1000 ) {
                    ...
                    do {
                        ...
                        if ( logIds != null && logIds.size() > 0 ) XxlJobAdminConfig.getAdminConfig().getXxlJobLogDao().deleteBatchIds(logIds);
